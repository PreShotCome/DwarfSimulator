...
export class Store extends EventEmitter {
  ...
  registerWorker(input: RegisterInput): Worker {
    ...
    this.db
      .prepare(
        `INSERT INTO workers
           (id, name, host, threads, pool, created_at, last_seen, status)
         VALUES (?, ?, ?, ?, ?, ?, ?, 'online')`,
      )
      .run(id, input.name, input.host, input.threads, input.pool, now, now);
    this.emit('change');
  ...
  heartbeat(id: string, input: HeartbeatInput): Worker | null {
    ...
    const result = this.db
      .prepare(
        `UPDATE workers
            SET last_seen = ?, status = 'online', hashrate = ?,
                accepted = ?, rejected = ?, uptime_seconds = ?
          WHERE id = ?`,
      )
      .run(now, input.hashrate, input.accepted, input.rejected, input.uptimeSeconds, id);
    if (result.changes === 0) {
      return null;
    }
    this.db
      .prepare(
        `INSERT INTO samples
           (worker_id, ts, hashrate, accepted, rejected, uptime_seconds)
         VALUES (?, ?, ?, ?, ?, ?)`,
      )
      .run(id, now, input.hashrate, input.accepted, input.rejected, input.uptimeSeconds);
    this.emit('change');
    return this.getWorker(id);
  }
  ...
  getHistory(id: string, sinceMs: number): Sample[] {
    return this.db
      .prepare(
        `SELECT ts, hashrate, accepted, rejected, uptime_seconds AS uptimeSeconds
           FROM samples
  ...
  summary(): Summary {
    ...
    return {
      ...
      totalHashrate: online.reduce((sum, w) => sum + w.hashrate, 0),
